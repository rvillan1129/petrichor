...
class PlantInstanceInline(admin.StackedInline):
    model = PlantInstance
    extra = 0
    fields = ['nickname', 'location', ('purchased', 'due_watered', 'snooze'), 'id']
    readonly_fields = ['nickname', 'location', 'purchased', 'due_watered', 'id', 'snooze']
...
class PlantInstanceAdmin(admin.ModelAdmin):
    list_display = ('nickname', 'plant', 'display_common_name', 'customer','location', 'due_watered', 'snooze')
    list_filter = ('due_watered',)

    fields = ['plant', 'nickname', 'location', 'customer', ('purchased', 'due_watered', 'snooze'), 'id']
    readonly_fields = ['id']
